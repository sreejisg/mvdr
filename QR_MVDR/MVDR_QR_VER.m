...
%    rf_data=rcvdata(1+((scan-1)*(768*2)):scan*(768*2),:);
     rf_data=rcvdata(1+(((62-i)-1)*(1536*1)):(62-i)*(1536*1),:);
...
%    end
  for ibin=1:Nbin
%       ibin=Nbin-ibin+1;
      StVect(:,ibin)=exp(j*2e-04*pi*(1/Nfft)*ibin*tauTxFocus);
   end
...
%         sv=StVect(:,i);
       ...
       sv1_z=zeros(nn,1);
  for kk=1:nn
    if (S_tran(kk,kk)==0) error('Matrix is singular!'); end;
    sv1_z(kk)=sv1(kk)/S_tran(kk,kk);
    sv1(kk+1:nn)=sv1(kk+1:nn)-S_tran(kk+1:nn,kk)*sv1_z(kk);
  end 
       %R= (R+eye(L,L));
       %RiE= (R\sv1);
       W=sv1_z/(sv1_z'*sv1_z);
       ...
       x_mean= zeros(L,1);
       x_mean=1/(N-L+1)*sum(U',2);
       sv1=x_mean;
       %Zmean=forwardSubstitution(S_tran,sv1,nn); 
       Zmean=zeros(nn,1);
        for kk=1:nn
            if (S_tran(kk,kk)==0) error('Matrix is singular!'); end;
             Zmean(kk)=sv1(kk)/S_tran(kk,kk);
            sv1(kk+1:nn)=sv1(kk+1:nn)-S_tran(kk+1:nn,kk)*Zmean(kk);
        end 
       MVDRY(k) = W'*Zmean;
     end
